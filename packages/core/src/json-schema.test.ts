...
function toolWith(input: z.ZodType, extra: Partial<ToolDefinition> = {}): ToolDefinition {
  return { name: 'test.tool', execution: 'remote', input, output: z.object({}), ...extra } as ToolDefinition
}
...
describe('toToolSchema', () => {
  ...
  it('不支持的类型抛错并指出工具名与字段路径', () => {
    ...
    expect(() => toToolSchema(toolWith(input, { name: 'browser.click' }))).toThrowError(/browser\.click.*when/)
  })
