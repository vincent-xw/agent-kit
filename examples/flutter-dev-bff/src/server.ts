...
export async function createFlutterDevBff(options: {
  ...
  // 文档页面（由 Markdown 插件转换生成，放在 public/ 下）
  const DOC_PAGES = ['guide', 'tools', 'skills', 'settings', 'companion', 'custom-tools']
  for (const page of DOC_PAGES) {
    app.get(`/${page}.html`, (c) => {
      const filePath = join(publicDir, `${page}.html`)
...
function ensureEnvTemplate(): void {
  ...
  const template = [
    ...
    '# 总开关：1=开启写日志文件；0=关闭',
    '# LOG_TO_FILE=1',
    '# 日志目录（默认 BFF 数据目录/log；相对路径以数据目录为基准，如 log → <数据目录>/log）',
    '# LOG_DIR=log',
    '# LOG_FORMAT 支持的 3 种：',
