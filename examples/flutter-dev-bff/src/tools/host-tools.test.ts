...
async function runApproved(
  exec: (i: unknown, c: unknown) => Promise<Record<string, unknown>>,
  input: unknown,
  ask: ReturnType<typeof createAskService>,
  callId: string,
  answer: string,
): Promise<Record<string, unknown>> {
  const p = exec(input, { sessionId: 'flutter-dev:s1', callId })
...
describe('host 工具', () => {
  ...
  it('host_file_list 列出目录条目', async () => {
    ...
    const out = await (t.execute as (i: unknown) => Promise<Record<string, unknown>>)({ path: '.' }, {})
  ...
  it('host_file_read 读文本', async () => {
    ...
    const out = await (t.execute as (i: unknown) => Promise<Record<string, unknown>>)({ path: 'a.txt' }, {})
  ...
  it('host_file_write 未受信任需审批，拒绝返回 denied', async () => {
    ...
    const out = await runApproved(t.execute, { path: 'b.txt', content: 'x' }, ask, 'c1', '拒绝')
  ...
  it('host_exec 未受信任返回 denied', async () => {
    const { svcs, ask } = makeSvcs(dir)
    const t = byName(createHostToolDefinitions(svcs), 'host_exec')
    const out = await runApproved(t.execute, { command: 'ls' }, ask, 'c1', '拒绝')
    expect(out.denied).toBe(true)
  })
  ...
  it('host_notify 返回 ok', async () => {
    ...
    const out = await (t.execute as (i: unknown) => Promise<Record<string, unknown>>)({ title: 't', message: 'm' }, {})
  ...
  it('web_fetch 本地回环被拦截', async () => {
    ...
    const out = await (t.execute as (i: unknown) => Promise<Record<string, unknown>>)({ url: 'http://127.0.0.1/' }, {})
    expect(out.blocked).toBe(true)
  })

  it('host_clipboard read 返回 ok', async () => {
    const { svcs } = makeSvcs(dir)
    const t = byName(createHostToolDefinitions(svcs), 'host_clipboard')
    const out = await (t.execute as (i: unknown) => Promise<Record<string, unknown>>)({ action: 'read' }, {})
